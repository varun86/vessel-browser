...
export function handleCreateCheckpoint(ctx: ActionContext, args: Record<string, unknown>): string {
  const checkpoint = ctx.runtime.createCheckpoint(args.name, args.note);
  return `Created checkpoint ${checkpoint.name} (${checkpoint.id})`;
}

export function handleRestoreCheckpoint(ctx: ActionContext, args: Record<string, unknown>): string {
